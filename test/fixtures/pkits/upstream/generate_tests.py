...
def output_test(
    ...
    |sanitized_test_names|."""
    sanitized_test_name = "Section%s%s" % (
        test_number.split(".")[1],
...
"""
    ...
    if info.initial_explicit_policy != default_info.initial_explicit_policy:
        output.write(
            """  info.SetInitialExplicitPolicy(%s);
"""
            % bool_to_str(info.initial_explicit_policy)
        )

    if (
        info.initial_policy_mapping_inhibit
        != default_info.initial_policy_mapping_inhibit
    ):
        output.write(
            """  info.SetInitialPolicyMappingInhibit(%s);
"""
            % bool_to_str(info.initial_policy_mapping_inhibit)
        )

    if info.initial_inhibit_any_policy != default_info.initial_inhibit_any_policy:
        output.write(
            """  info.SetInitialInhibitAnyPolicy(%s);
"""
    ...
    if info.user_constrained_policy_set != default_info.user_constrained_policy_set:
        output.write(
            """  info.SetUserConstrainedPolicySet(%s);
"""
...
USING_DEFAULT_SETTINGS_MATCHER = re.compile("^.*using the \s*default settings.*")
...
TEST_OVERRIDES = {
    ...
    "4.8.6": [  # Overlapping Policies Test6
        ...
        TestInfo(
            False, initial_policy_set=[TEST_POLICY_2], user_constrained_policy_set=[]
        ),
    ],
    ...
    "4.8.13": [  # All Certificates Same Policies Test13
        ...
        TestInfo(
            True,
            initial_policy_set=[TEST_POLICY_3],
    ...
    "4.10.2": [  # Invalid Policy Mapping Test2
        ...
        # path should not validate successfully.
        TestInfo(
            False, initial_policy_mapping_inhibit=True, user_constrained_policy_set=[]
        ),
    ],
    "4.10.3": [  # Valid Policy Mapping Test3
        ...
        # The path should not validate successfully.
        TestInfo(
            False, initial_policy_set=[TEST_POLICY_1], user_constrained_policy_set=[]
        ),
        # 2. default settings, but with initial-policy-set = {NIST-test-policy-2}.
        # The path should validate successfully.
        TestInfo(
    ...
    "4.10.5": [  # Valid Policy Mapping Test5
        ...
        # The path should not validate successfully.
        TestInfo(
            False, initial_policy_set=[TEST_POLICY_6], user_constrained_policy_set=[]
        ),
    ],
    ...
    "4.12.3": [  # inhibitAnyPolicy Test3
        ...
        # should not validate successfully.
        TestInfo(
            False, initial_inhibit_any_policy=True, user_constrained_policy_set=[]
        ),
    ],
...
def parse_test(
    lines, i, test_case_name, test_number, test_name, sanitized_test_names, output
):
    ...
    if overrides is None:
        ...
        if not USING_DEFAULT_SETTINGS_MATCHER.match(
            ...
            sys.stderr.write(
                "Unexpected procedure for %s: %s\n"
                % (test_number, " ".join(test_section.procedure_lines))
            )
            sys.exit(1)

        # Check whether expected result is validation success or failure.
        result_match = TEST_RESULT_MATCHER.match(test_sections.expected_result_lines[0])
        if not result_match:
            sys.stderr.write(
                "Unknown expectation for %s:\n%s\n"
...
def main():
    ...
    test_descriptions = test_descriptions.split(
        "5 Relationship to Previous Test Suite", 1
    )[0]
...
// NOLINT(build/header_guard)\n\n""")
    ...
    while i < len(lines):
        ...
        if section_match:
            ...
            output.write(
                "class %s : public PkitsTest<PkitsTestDelegate> {};\n" % test_case_name
            )
