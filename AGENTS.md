...
- `packages/` holds workspace tooling (doc-render, VitePress plugins). It, `scripts/` and `.claude/` are excluded from the coverage gate (`bunfig.toml` `coveragePathIgnorePatterns`), which measures `src/` only.
