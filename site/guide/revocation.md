...
RFC 5280 requires conforming CRL issuers to include `nextUpdate`, and leaves
client handling of a CRL without one unspecified. By default micro509 applies
no expiry to such a CRL, so a replayed pre-revocation CRL would still validate.
Set `maxAgeMs` to bound how old `thisUpdate` may be; the same knob is
